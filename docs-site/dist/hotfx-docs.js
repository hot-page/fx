import"https://cdn.jsdelivr.net/npm/prismjs@1.29.0/prism.min.js";import{marked as f}from"https://cdn.jsdelivr.net/npm/marked@12.0.2/lib/marked.esm.min.js";let l;class m extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.#M(),this.#h()}toggleHTML(){this.shadowRoot.querySelector("footer").classList.toggle("html-expanded")}async#h(){const z=await(await fetch(this.getAttribute("src")+".css")).text(),r=await(await fetch(this.getAttribute("src")+"?theRealThingAndNotSomeClientRenderedBS")).text(),M=new DOMParser().parseFromString(r,"text/html");M.querySelectorAll('script:not([src*="jsdelivr"]):not([src*="localhost"])').forEach(h=>h.remove());const t=M.querySelector("head style").innerHTML.split(`
`).map(h=>h.trim()).filter(h=>h&&!h.startsWith(`@import url('${this.getAttribute("src")}`)).map(h=>`  ${h.replace(/\?cache-key=[^']*'/,"'").trim()}`).join(`
`);if(!this.querySelector('[slot="source"]')){const h=Array.from(M.querySelectorAll("script")).map(a=>a.outerHTML);this.shadowRoot.querySelector("#source code").textContent+=h.join(`

`),this.shadowRoot.querySelector("#source code").textContent+=h.length?`
`:"";let o=`<style>
`;o+=t,o+=t.length&&z.length?`

`:"",o+=z.split(`
`).map(a=>`  ${a}`).join(`
`),o+=`
</style>
`,this.shadowRoot.querySelector("#source code").textContent+=o;const d=r.indexOf("<body>")+6,c=r.indexOf("</body>"),p=r.slice(d,c).split(`
`).map(a=>a.startsWith("    ")?a.slice(4):a).join(`
`);this.shadowRoot.querySelector("#source code").textContent+=p}Prism.highlightElement(this.shadowRoot.querySelector("#source code"));const H=this.shadowRoot.querySelector("#body");if(this.hasAttribute("use-iframe")){const h=document.createElement("iframe");h.srcdoc=M.documentElement.outerHTML.replaceAll("@import url('/","@import url('https://fx.hot.page/"),h.setAttribute("part","iframe"),H.appendChild(h)}else H.attachShadow({mode:"open"}),H.shadowRoot.innerHTML=`
        ${M.querySelector("head style").outerHTML.replaceAll("url('/","url('https://fx.hot.page/")}
        ${M.body.innerHTML}
      `}#v=()=>{const v=document.createElement("pre");v.style.width="1px",v.style.height="1px",v.style.position="fixed",v.style.top="5px",v.textContent=this.shadowRoot.querySelector("#source").textContent.trim(),document.body.appendChild(v);const z=getSelection();z.removeAllRanges();const e=document.createRange();e.selectNodeContents(v),z.addRange(e),document.execCommand("copy"),z.removeAllRanges(),document.body.removeChild(v),this.shadowRoot.querySelector("#copy span").textContent="Copied!",setTimeout(()=>{this.shadowRoot.querySelector("#copy span").textContent="Copy"},2e3)};#M(){this.shadowRoot.innerHTML=`
    ...
    `,this.shadowRoot.querySelector("button.html").addEventListener("click",()=>this.toggleHTML()),this.shadowRoot.querySelector("#copy").addEventListener("click",this.#v)}}setTimeout(()=>customElements.define("hotfx-demo",m));l=`
...
`;class u extends HTMLElement{#h;constructor(){super(),this.attachShadow({mode:"open"}),this.#v()}copy(){const v=document.createElement("pre");v.style.width="1px",v.style.height="1px",v.style.position="fixed",v.style.top="5px",v.textContent=this.#h,document.body.appendChild(v);const z=getSelection();z.removeAllRanges();const e=document.createRange();e.selectNodeContents(v),z.addRange(e),document.execCommand("copy"),z.removeAllRanges(),document.body.removeChild(v),this.shadowRoot.querySelector("#container").classList.add("copied"),setTimeout(()=>{this.shadowRoot.querySelector("#container").classList.remove("copied")},2e3)}async#v(){const v=this.getAttribute("package"),e=await(await fetch(`https://registry.npmjs.org/${v}/latest`)).json(),r=this.getAttribute("file")?"/"+this.getAttribute("file"):"";this.#h=`https://cdn.jsdelivr.net/npm/${v}@${e.version}${r}`,this.shadowRoot.innerHTML=`
      <style>
        :host {
      ...
      <div id="container">
        <span id="link" part="link">${this.#h}</span>
    ...
    `,this.shadowRoot.querySelector("#copy").addEventListener("click",()=>this.copy())}}customElements.define("hotfx-npm-permalink",u);let n="";class g extends HTMLElement{constructor(){super(),this.innerHTML=`
      <style>
        hotfx-js-docs div {
      ...
      <div></div>
    `,this.#h()}async#h(){const z=await(await fetch(this.getAttribute("src"))).text(),e=Prism.tokenize(z,Prism.languages.javascript),i=e.reduce((M,t,H)=>{let h=M[M.length-1];const o=e[H+1];return h.type=="code"&&t.type=="comment"?(h={type:"comment",tokens:[]},M.push(h)):h.type=="comment"&&(typeof t=="string"&&/\S/.test(t)||typeof t=="object"&&t.type!="comment"||typeof o=="object"&&o.type!="comment")&&(h={type:"code",tokens:[]},M.push(h)),h.tokens.push(t),M},[{type:"comment",tokens:[]}]).map((M,t)=>{if(M.type=="comment"){const H=M.tokens.map(h=>typeof h=="object"?h.content:h).join("").replaceAll(/^\s*\/\* ?/mg,"").replaceAll(/^\s*\*\s ?/mg,"").replaceAll(/^\s*\/\/ ?/mg,"").replaceAll(/\*\/\s*$/mg,"").replace(/(\s|$)HTML([\s.])/g,"$1<abbr>HTML</abbr>$2").replace(/(\s|$)CSS([\s.])/g,"$1<abbr>CSS</abbr>$2").replace(/(\s|$)DOM([\s.])/g,"$1<abbr>DOM</abbr>$2").replace(/(\s|$)URL([\s.])/g,"$1<abbr>URL</abbr>$2").replace(/(\s|$)GET([\s.])/g,"$1<small>GET</small>$2").replace(/(\s|$)POST([\s.])/g,"$1<small>POST</small>$2");return`
          <div class="hotfx-js-docs-row" id="section-${t/2}">
            <div class="comment-section">
              <a class="section-link" href="#section-${t/2}">${n}</a>
              ${f.parse(H)}
            </div>`}else if(M.type=="code")return`
            <pre class="code-section" class="language-javascript"><code>${Prism.Token.stringify(Prism.util.encode(M.tokens),"javascript").replace(/^\n/,"").replace(/[ ]*$/,"")}</code></pre>
          </div>`}).join("");this.innerHTML=i,Array.from(this.querySelectorAll('.hotfx-js-docs-row .comment-section code[class*="language-"]')).forEach(M=>Prism.highlightElement(M))}}customElements.define("hotfx-js-docs",g);n='<svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 640 512" height="20px" width="20px" xmlns="http://www.w3.org/2000/svg"><path d="M579.8 267.7c56.5-56.5 56.5-148 0-204.5c-50-50-128.8-56.5-186.3-15.4l-1.6 1.1c-14.4 10.3-17.7 30.3-7.4 44.6s30.3 17.7 44.6 7.4l1.6-1.1c32.1-22.9 76-19.3 103.8 8.6c31.5 31.5 31.5 82.5 0 114L422.3 334.8c-31.5 31.5-82.5 31.5-114 0c-27.9-27.9-31.5-71.8-8.6-103.8l1.1-1.6c10.3-14.4 6.9-34.4-7.4-44.6s-34.4-6.9-44.6 7.4l-1.1 1.6C206.5 251.2 213 330 263 380c56.5 56.5 148 56.5 204.5 0L579.8 267.7zM60.2 244.3c-56.5 56.5-56.5 148 0 204.5c50 50 128.8 56.5 186.3 15.4l1.6-1.1c14.4-10.3 17.7-30.3 7.4-44.6s-30.3-17.7-44.6-7.4l-1.6 1.1c-32.1 22.9-76 19.3-103.8-8.6C74 372 74 321 105.5 289.5L217.7 177.2c31.5-31.5 82.5-31.5 114 0c27.9 27.9 31.5 71.8 8.6 103.9l-1.1 1.6c-10.3 14.4-6.9 34.4 7.4 44.6s34.4 6.9 44.6-7.4l1.1-1.6C433.5 260.8 427 182 377 132c-56.5-56.5-148-56.5-204.5 0L60.2 244.3z"></path></svg>';
//# sourceMappingURL=hotfx-docs.js.map
